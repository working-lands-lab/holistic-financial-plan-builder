...
// US accounting format: $ flush left, figures flush right, negatives in
// parentheses, zero shown as a dash.
const MONEY_FMT = '_("$"* #,##0_);_("$"* (#,##0);_("$"* "-"_);_(@_)';
const PCT_FMT = "0.0%";
const ACCENT = "FF2F6F4F";
const ACCENT_SOFT = "FFE7F1EB";
const HEAD_FILL = "FFEFF2F4";
const ZEBRA = "FFF8FAFB";
const RULE = "FF9AA3B2"; // total/section rules
const HAIRLINE = "FFCDD2DA";
const MUTED = "FF8B94A3";
...
// A formula cell carrying a cached result, so the number shows in viewers that
// don't recalculate (Quick Look, mail previews, the embedded site) — Excel,
// Numbers, and Sheets recompute it on open regardless.
function formula(f, result) {
  return result === undefined ? { formula: f } : { formula: f, result };
}

function titleCell(ws, addr, text) {
  const c = ws.getCell(addr);
  c.value = text;
  c.font = { bold: true, size: 15, color: { argb: ACCENT } };
  return c;
}

// Styles a header row. `numFrom` is the first column whose header should be
// right-aligned (to sit over numeric data); earlier columns stay left-aligned.
function headerRowStyle(ws, rowNumber, fromCol, toCol, numFrom = Infinity) {
  for (let i = fromCol; i <= toCol; i++) {
    ...
    c.alignment = {
      vertical: "middle",
      wrapText: true,
      horizontal: i >= numFrom ? "right" : "left",
    };
    c.border = { bottom: { style: "medium", color: { argb: RULE } } };
  }
  ws.getRow(rowNumber).height = 24;
}

// Draws a thin rule across the top of a row — the accounting "rule above the
// total" look.
function ruleAbove(ws, rowNumber, fromCol, toCol) {
  for (let i = fromCol; i <= toCol; i++) {
    const c = ws.getCell(`${colLetter(i)}${rowNumber}`);
...
function fillRow(ws, rowNumber, fromCol, toCol, argb) {
  for (let i = fromCol; i <= toCol; i++) {
    ws.getCell(`${colLetter(i)}${rowNumber}`).fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb },
    };
...
function buildEnterpriseSheet(wb, sheetName, displayName, items, plan, opts = {}) {
  ...
  rows.forEach((it, idx) => {
    ...
    ws.getCell(`D${r}`).value = formula(`SUM(E${r}:P${r})`, itemAnnual(it));
    monthly.forEach((v, m) => {
      const cell = ws.getCell(`${colLetter(5 + m)}${r}`);
      if (v !== 0) cell.value = v;
    });
    // Annual Total Actual = SUM(actual months)
    ws.getCell(`Q${r}`).value = formula(`SUM(R${r}:AC${r})`, 0);
  ...
  // Cached monthly + annual totals per type, so the SUMIF cells display in any
  // viewer. Keyed by type -> { months:[12], annual }.
  const typeTotals = {};
  typeRows.forEach((t) => {
    const months = new Array(12).fill(0);
    rows
      .filter((it) => it.type === t)
      .forEach((it) => monthlyArray(it).forEach((v, m) => (months[m] += v)));
    typeTotals[t] = { months, annual: months.reduce((a, b) => a + b, 0) };
  });

  const cols = [4, ...Array.from({ length: 12 }, (_, i) => 5 + i)];
  const cachedFor = (t, cc) => (cc === 4 ? typeTotals[t].annual : typeTotals[t].months[cc - 5]);
  ...
  typeRows.forEach((t) => {
    ...
    cols.forEach((cc) => {
      ...
      ws.getCell(`${L}${cursor}`).value = formula(
        `SUMIF($A$${firstItemRow}:$A$${safeLast},$A${cursor},${L}$${firstItemRow}:${L}$${safeLast})`,
        cachedFor(t, cc)
      );
      ws.getCell(`${L}${cursor}`).numFmt = MONEY_FMT;
    });
    ws.getCell(`D${cursor}`).font = { bold: true };
    cursor++;
  });
  ...
  cols.forEach((cc) => {
    const L = colLetter(cc);
    const cached = typeRows.reduce((s, t) => s + cachedFor(t, cc), 0);
    ws.getCell(`${L}${rtoRow}`).value = formula(
      `SUM(${L}${typeRowStart}:${L}${typeRowEnd})`,
      cached
    );
    ws.getCell(`${L}${rtoRow}`).numFmt = MONEY_FMT;
    ws.getCell(`${L}${rtoRow}`).font = { bold: true };
  });
  ruleAbove(ws, rtoRow, 1, 16);
  ...
  if (incomeIdx >= 0) {
    ...
    const incAnnual = typeTotals["Income"].annual;
    const rtoAnnual = typeRows.reduce((s, t) => s + typeTotals[t].annual, 0);
    ws.getCell(`D${rtoPctRow}`).value = formula(
      `IFERROR(D${rtoRow}/D${incRow},0)`,
      incAnnual ? rtoAnnual / incAnnual : 0
    );
...
function buildAllItemsTable(wb, plan) {
  ...
  flat.forEach(({ source, it }) => {
    ...
    ws.getCell(`E${r}`).value = formula(`SUM(F${r}:Q${r})`, itemAnnual(it));
    monthly.forEach((v, m) => {
  ...
  // Grand total row
  const grand = flat.reduce((s, { it }) => s + itemAnnual(it), 0);
  const gtRow = r + 1;
  ws.getCell(`C${gtRow}`).value = "Grand Total";
  ws.getCell(`C${gtRow}`).font = { bold: true };
  ws.getCell(`E${gtRow}`).value = formula(`SUM(E${dataStart}:E${dataEnd})`, grand);
  ws.getCell(`E${gtRow}`).numFmt = MONEY_FMT;
  ws.getCell(`E${gtRow}`).font = { bold: true };
  ruleAbove(ws, gtRow, 3, 5);
...
function buildSummariesSheet(wb, plan, ref) {
  ...
  const lines = [
    ["Income", formula(sumif("Income"), s.income), "Gross Income"],
    ["Inescapable Expense", formula(sumif("Inescapable Expense"), s.inescapable), ""],
    ["Logjam Expense", formula(sumif("Logjam Expense"), s.logjam), ""],
    ["Maintenance Expense", formula(sumif("Maintenance Expense"), s.maintenance), ""],
    [
      "Return to Overhead",
      formula("SUM(B5:B7)", s.returnToOverhead),
      "Income taken to cover Inescapable, Logjam, and Maintenance expenses.",
    ],
    [
      "Remaining Income",
      formula("SUM(B4,B8)", s.remainingIncome),
      "What's left after overheads. Must be positive.",
    ],
    [
      `Planned Profit (${Math.round(pct * 100)}%)`,
      formula(`${pct}*B9`, s.plannedProfit),
      "A non-negotiable amount taken out of remaining income.",
    ],
    [
      "Reinvestment Fund",
      formula("B9-B10", s.reinvestmentFund),
      "After profit, what's left to reinvest in weak links and adverse factors.",
    ],
    [
      "Wealth Generating Expenses",
      formula(sumif("Wealth Generating Expense"), s.wealth),
      "Total of Wealth Generating Expenses.",
    ],
    [
      "Net Income",
      formula("B12+B11", s.netIncome),
  ...
  const rowOf = {};
  let r = 4;
  lines.forEach(([label, val, comment]) => {
    rowOf[label.replace(/ \(.*/, "")] = r;
    ws.getCell(`A${r}`).value = label;
    ws.getCell(`B${r}`).value = val;
    ws.getCell(`B${r}`).numFmt = MONEY_FMT;
    ws.getCell(`C${r}`).value = comment;
    ws.getCell(`C${r}`).font = { size: 10, color: { argb: MUTED } };
  ...
  // Accounting rules above the running totals, and a highlight on the result.
  ruleAbove(ws, rowOf["Return to Overhead"], 1, 2);
  ruleAbove(ws, rowOf["Remaining Income"], 1, 2);
  ruleAbove(ws, rowOf["Net Income"], 1, 2);
  fillRow(ws, rowOf["Net Income"], 1, 3, ACCENT_SOFT);
  ws.getCell(`A${rowOf["Net Income"]}`).font = { bold: true, color: { argb: ACCENT } };
  ws.getCell(`B${rowOf["Net Income"]}`).font = { bold: true, color: { argb: ACCENT } };
  ...
  TYPE_LIST.forEach((t) => {
    ...
    ws.getCell(`B${r}`).value = formula(sumif(t), totalByType(plan, t));
    ws.getCell(`B${r}`).numFmt = MONEY_FMT;
    r++;
  });
